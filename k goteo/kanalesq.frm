...
Rem Do
   ...
   Rem  p = B + 2 * Y * l
   Rem  T = B + 2 * Y * Z
    Rem fmichi = q - A ^ (5 / 3) / p ^ (2 / 3) / n * Sqr(s)
    ...
    Rem dy = 0.001 * DFMICHI / (DFMICHI - fmichi)
    Rem Y = Y - dy + 0.001
Rem Loop While Abs(dy) >= 0.00001
Rem GoSub NORMAL
areakkk = Y * (B + Y * Z)
p = B + 2 * Y * l
T = B + 2 * Y * Z
